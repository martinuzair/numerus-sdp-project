...
if ($level && $chapterID && $subtopicID) {
    if (isset($_POST['submit'])) {
        ...
        if (isset($_FILES['myFile']['name']) && $_FILES['myFile']['name'] != '') {
            $target_dir = "C://xampp//htdocs//Children2//tutorial_notes//";
                ...
                if ($_FILES['myFile']['size'] >= $maxsize) {
                    ...
                    if ($_FILES['myFile']['error'] === UPLOAD_ERR_OK) {
                        if (move_uploaded_file($_FILES['myFile']['tmp_name'], $target_file)) {
                            try {
                            $query = "UPDATE subtopic SET Notes = :note WHERE Subtopic_ID = :subtopicID";                                
                            $stmt = $pdo->prepare($query);
                            $stmt->bindParam(':note', $relative_path);
            ...
            $_SESSION['message'] = "Invalid file extension. Only pdf is allowed.";
        }
    } else {
